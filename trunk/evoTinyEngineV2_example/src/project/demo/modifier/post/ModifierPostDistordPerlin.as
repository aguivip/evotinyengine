package project.demo.modifier.post
{
	...
	public class ModifierPostDistordPerlin extends AbstractModifier
	{
		...
		public function ModifierPostDistordPerlin(amount:int = 100, quality:int = 2, baseXY:Point = null, increase:Boolean = false, decrease:Boolean = false, id:String = "")
		{
			super(id);
		...
		override public function render(data:RenderData):void
		{
			if(increase) 
			{
				amount = amountGoal * data.lifeTime;
			}
			else if(decrease)
			{
				amount = amountGoal * (1-data.lifeTime);
			}
