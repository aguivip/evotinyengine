package project.demo.modifier.post
{
	 ...
	 */
	public class ModifierPostFishEye extends AbstractModifier
	{
		...
		public function ModifierPostFishEye(amount:int = 100, radius:Number = 200, dirXY:Point = null, increase:Boolean = false, id:String = "")
		{
			super(id);
		...
		override public function render(data:RenderData):void
		{
			...
			if(_assets.channel.leftPeak < .1 && _assets.channel.rightPeak < .1) return
			
			offX = offX + (_assets.channel.leftPeak - offX) * .5;
			offY = offY + (_assets.channel.rightPeak - offY) * .5;
			
			filter.scaleX = amount * offX * dirXY.x;
			filter.scaleY = amount * offY * dirXY.y;
